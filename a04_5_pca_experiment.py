...
def robust_pca(D, lam=None, mu=None, max_iter=1000, tol=1e-7):
    """
    Robust PCA via Principal Component Pursuit (Candes et al.)
    Decomposes D ≈ L + S
    Returns: low-rank L, sparse S
    """
    m, n = D.shape
    if lam is None:
        lam = 1.0 / np.sqrt(max(m, n))
    if mu is None:
        mu = (m * n) / (4.0 * norm(D, ord=1))

    L = np.zeros_like(D)
    S = np.zeros_like(D)
    Y = D / max(norm(D, 2), norm(D, np.inf) / lam)

    for _ in range(max_iter):
        # Low-rank update via SVD thresholding
        U, sigma, VT = svd(D - S + (1 / mu) * Y, full_matrices=False)
...
if __name__ == '__main__':
    ...
    for i in range(len(insect_classes)):
        ...
        # --- Apply Robust PCA ---
        L, S = robust_pca(latents_cnn_train)

        # --- Reduce to 10D with standard PCA ---
        pca = PCA(n_components=10)
        X_rpca_10d = pca.fit_transform(L)

        # --- Plot all 45 pairwise combinations of 10D ---
        ...
        for idx, (i, j) in enumerate(combinations):
            ax = axes[idx]
            ax.scatter(X_rpca_10d[:, i], X_rpca_10d[:, j], alpha=0.5, s=5)
            ax.set_xlabel(f'Dim {i}')
            ax.set_ylabel(f'Dim {j}')
            ax.set_title(f'Dim {i} vs {j}')
