...
if __name__ == '__main__':
    ...
    for vq_vae_type in vq_vae_types:
        for i in range(len(insect_classes)):
            ...
            os.makedirs(umap_folder, exist_ok=True)
            umap_train_file_name = os.path.join(umap_folder, f'umap_vect_{main_insect_class}_{vq_vae_type}ae_train.npy')
            umap_test_file_name = os.path.join(umap_folder, f'umap_vect_{main_insect_class}_{vq_vae_type}ae_test.npy')
            labels_umap_train_file_name = os.path.join(umap_folder, f'labels_umap_vect_{main_insect_class}_{vq_vae_type}ae_train.npy')
            labels_umap_test_file_name = os.path.join(umap_folder, f'labels_umap_vect_{main_insect_class}_{vq_vae_type}ae_test.npy')
            ...
            visualize_train_latent_space(
                latents_2d_train, 
                labels_noise_train, 
                f'ae_{vq_vae_type}{main_insect_class}_train', 
                config["logging_params"]["save_dir"]
            )
